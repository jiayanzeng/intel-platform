...
**v0.43 Amendment A3 / LEDGER 2e — unconditional append-order
supersession completed (measured 2026-08-05).** This measurement supersedes
the A2 contract-change gate and its DOC-BOUNDARY blocker. Every checked box with
multiple valid runbook-qualified entries now attests the last entry by file
position without reading amendment disclosure or contract fields. Every earlier
entry remains in the append-only log, and an unresolvable entry remains a
finding even when a later valid record exists. Before the required fourth
LEDGER audit entry exists, the checked live case resolves three qualified
records to `8c7ee461e37ce6008c77d7dc841732a7dcd580c4`, with two superseded,
at **335 checked / 3 retracted / 326 matched / 326 resolved / 9 exemptions**.

R13's nine controls cover the four standing population/correspondence cases,
zero valid candidates, an invalid entry followed by a valid one, unrecoverable
entry order, unconditional duplicate resolution, and a same-date counterexample
whose append order opposes commit sorting. The complete registered self-test
passes **17/17 rules / 128/128 controls**. The permission-capable checked-state
matrix passes **22/22**; Python 3.11.4 and 3.12.13 pass **403/403** each against
the exact **21-package** constraints with the same single known Starlette
deprecation warning. Embedded and standalone golden pass **11/11**, for delta
zero. `cycle-check` measures **35 ledger subjects** and the retained
cycle-document set at **162,875 raw bytes / 4 paths** against the executable
**197,953-byte** boundary, leaving **35,078 bytes**. No progress entry,
exemption, retraction, dependency, production source, protected byte, public
surface, tag, branch, or remote ref moved.

**v0.43 DOC-BOUNDARY — completed under A3 append-order correspondence
(measured 2026-08-05).** The existing boundary implementation is unchanged.
With its box checked and before the fourth completion record exists,
`checklist-audit` resolves its three qualified records to the third,
`ca6cc544ffbdbc797ff314ec8a8c5816d4a66932`, while retaining the first two as
superseded history. LEDGER independently resolves four records to
`3d47bf5ede1083a62a43052ad37b9f7169794fa1`. Aggregate correspondence is
**336 checked / 3 retracted / 327 matched / 327 resolved / 9 exemptions**.

The permission-capable checked-state matrix passes **22/22**, including the
complete **17/17-rule / 128-control** self-test, Python 3.11.4 at **403/403**,
and embedded golden at **11/11**. The immediately preceding Python 3.12.13 run
over the same source passes **403/403** against all **21** constraints, and the
required standalone golden rerun passes **11/11**. The only Python warning in
either population remains the known Starlette deprecation; Rust is
warning-clean. `cycle-check` measures **166,963 raw retained cycle-document
bytes / 4 paths** against the executable **197,953-byte** boundary, leaving
**30,990 bytes**. The crossing trigger did not fire. No prior progress byte,
exemption, retraction, dependency, production source, protected byte, public
surface, tag, branch, or remote ref moved.

**v0.43 PUBLISH — Grant G not granted; published history remains incomplete
(measured 2026-08-05).** Every operator message through this step was checked
against Grant G's exact required text. None issued that authority. No push was
attempted, and no branch or tag moved.

Permission-capable direct readback returns exact remote
`main=993813c755e9f759a4ee165954c7a1df984f6b10` and no direct or peeled
`v0.17.9` or `v0.17.6` tag. Pre-disposition HEAD
`876cd9a561d81965b48449ed2b4f07bed438ed47` is the published main's **41
first-parent-commit descendant**. A reader of published history therefore
cannot see v0.41's audit child `827192d…`, any v0.42 commit through exact audit
child `6f07edf84c3ce40f1ef4c9e97e5d101242490243`, any v0.43 commit, or the local
annotated v0.17.9 release. The local tag remains exact at object
`a7852c55deba9b509c0235dfba38e2a0426c2501`, peeling to closing commit
`0382622bbfaeaf7092830460d6432a2eb777b031`.

R15 remains exact at **0 public response-domain differences / 6 routes / 31
status-media variants / 112 field occurrences**. The unpublished distance
continues to contain lifecycle, checker, governing-record, and release-authority
changes only; it carries no measured runtime-behavior difference or public
surface movement, so the current publication-epoch count remains **0**. The
local v0.42 progress record still carries its ordered closing-tree export audit,
and R17 confirms published `main` still cannot see it. The required standalone
golden run passes **11/11** with zero delta. No Step 5 stop condition fired.

**v0.43 ARCHIVE — Grant E not granted; second archival remains dormant
(measured 2026-08-05).** Every operator message through this step was checked
against Grant E's exact required text. None issued that authority, so this is
the runbook's dated not-granted branch rather than an inferred refusal or a
partial archival.

The directly delimited v0.38-to-permanent-marker region remains **84,896
unrecovered bytes**. Before this observation, State measured **245,192 /
453,741 bytes**. No file was added under `docs/state-archive/`; the existing
four archives remain the complete population. The protected manifest remains
exact at **200,440 bytes / 339 pins**, SHA-256
`f59d4520cfaa0190954442856b6bb1ab5576049f16d5b8b816f00f64495fefae`.

The pre-disposition project-root export passes at **2,805,106 bytes / 158
tracked entries / 2 retained cycles**, **235,718 bytes above** the executable
**2,569,388-byte** attention boundary and **194,894 bytes / 6.50% / 0.91
high-water cycles** below the fixed ceiling. Because attention remains fired,
the archival observation uses the substantive form
`kind=unheld-lever`, `lever=Grant E`, and `recoverable_bytes=84896`; it claims
neither an archive nor a pin. The checked-state permission-capable matrix
passes **22/22**, including **17/17 rules / 128 controls**, Python 3.11.4 at
**403/403**, and embedded golden at **11/11**. Standalone golden also passes
**11/11** with zero delta. No Step 4 stop condition fired.
...
boundary implementation passes the focused cycle/version suite **115/115** and
the complete registry **17/17 rules / 123 controls**. `cycle-check` measures the
Git-derived retained set at **149,990 raw bytes / 4 paths** against the
executable **197,953-byte** boundary, leaving **47,963 bytes**; the crossing
trigger is not fired. `version-check` passes over all **596** tracked files.

The required full matrix then stops at checked-task evidence. The original
LEDGER entry at `PROGRESS-v0.43.md:110` and the Amendment A1 tracked-state
completion at line 203 are both valid, runbook-qualified implementation
records, so `checklist-audit` reports `multiple valid runbook-qualified
entries`. Release/version consistency and active-cycle consistency passed
before that failure. The collision is a second acceptance-shape defect: the
runbook required the task to reopen and the standing workflow required a new
append-only post-completion record, while the checker permits only one qualified
record per checked task.

No existing progress byte is rewritten. `tools/checklist_audit.py` is outside
declared scope, and no exemption or retraction is added to silence the finding;
the standing contract forbids reducing a control's reach to make a failure
disappear and requires an operator decision before any retraction. DOC-BOUNDARY
therefore remains open and Steps 4–7 remain unexecuted. No dependency,
production source, protected byte, public value domain, publisher wire, local
tag, branch, or remote ref moved.

**v0.43 Amendment A1 / LEDGER 2c — tracked net-floor classification
completed (measured 2026-08-05).** Amendment A1 admits exactly
`tools/version_check.py` for the ledger classification and no other purpose.
Tracked candidate `4a5c0e27229a3616708a8e4671cb811967024bf1` implements the
current-restatement choice: the live ledger subject and trigger remain at Rust
1.86, while the current v0.43 observation names the pinned net-floor result
without repeating either floor numeral. The ledger is not placed in the
historical family; no closed dated measurement is edited.

At that exact committed state, `version-check` derives **3** offline authorities
at 1.78 and **2** net authorities at 1.86, binds **22** offline plus **2** ledger
current restatements, and classifies **596** tracked files / **103**
literal-bearing files / **1,056** literal occurrences. The ledger's selected
membership is `registered current restatement`. A planted move of both net
authorities to 1.87 fails specifically because `docs/DEFERRED.md` still states
1.86; disabling that comparison is R12 self-test **45/67**. The focused version
suite passes **16/16** and the complete registry passes **17/17 rules / 123
controls**.

The permission-capable full local matrix passes **22/22**, including zero-warning
offline and net Rust lanes, Rust 1.78 success, net Rust 1.86 success and 1.85
refusal, clippy, formatting, protected artifacts, Python 3.11 at **403/403**, and
embedded golden at **11/11**. The independently executed Python 3.12 population
passes **403/403** with the same accepted non-fatal warning. `cycle-check`
measures the canonical ledger at **33 subjects / 11,383 bytes**, keeps retention
at **2 cycles** with `retention_change=not-granted`, and leaves **50,581 bytes**
below the governed cycle-document boundary. The two amendment-required
enumeration subjects carry dated v0.43 observations and action `none`. The three
protected untracked amendment inputs remain untouched; no dependency,
production source, protected byte, public value domain, publisher wire, tag,
branch, or remote ref moved. This forward-corrects the earlier tracked-state
blocker without rewriting its accurate-at-the-time record.

**v0.43 DOC-BOUNDARY declared-scope gate — step halted (measured
2026-08-05).** After implementation commit
`5816bc69a84d6fa652f7d88bbebcb6008b70cd8f` existed, the required
`cycle-check` measured `declared scope diff rejects tools/version_check.py`.
That path is neither allowed nor a release authority in v0.43. The scope is not
widened and the gate is not silenced. Step 3 is reopened and halted; the
out-of-scope classification edit is removed in a forward corrective commit.

The cause also corrects the prior LEDGER acceptance claim. Its pre-commit full
matrix ran while new `docs/DEFERRED.md` was not yet Git-tracked, so
`version-check`'s `git ls-files` population did not examine the construction.
Once LEDGER was committed, the stable file's carried Rust 1.86/1.85 trigger and
dated observation entered the tracked floor-literal population without a
file-level classification. The DOC-BOUNDARY implementation proved that adding
`docs/DEFERRED.md` to the historical family would make `version-check` and the
full matrix pass, but that proof changed the undeclared checker path and cannot
be adopted in this cycle. With the correction removed, `version-check` again
truthfully rejects the ledger. DOC-BOUNDARY therefore cannot satisfy its clean
`./run ci-local` acceptance inside declared scope. No dependency, production
source, protected byte, public value domain, publisher wire, tag, branch, or
remote ref moved. This is a scope/measurement blocker, not a reason to rewrite
the carried ledger trigger or amend scope after activation.

**v0.43 DOC-BOUNDARY — retained review-output set governed (measured
2026-08-05).** C33 selects the exact Git-derived retained review-export set:
the active and immediately prior runbook/progress documents. At clean baseline
tree `f8407acaf3fdc3aa4b330950217489f2c117cb62`, the permission-capable export
measured **2,726,105 bytes / 158 tracked entries / 2 retained cycles**. The four
retained cycle documents contributed **139,364 Repomix content bytes**, leaving
a measured **2,586,741-byte non-cycle payload**. Preserving the maximum positive
adjacent governed export delta of **215,306 bytes** derives the boundary as
`3,000,000 - 2,586,741 - 215,306 = 197,953 bytes`, strictly inside the fixed
ceiling. Raw repository bytes are the conservative live measure because they
include the four terminal newlines removed by Repomix.

After the C33 decision record, `cycle-check` measures **140,818 raw bytes / 4
paths**, leaving **57,135 bytes** of headroom, and re-derives the same reserve
and boundary from repository authorities. The trigger did not fire. The new
predicate identifier lives in the existing governed-trigger registry. R12's
planted mutation disables the crossing gate and is detected; direct focused
construction rejects equality without a dated substantive disposition and
accepts the same equality with the existing `unheld-lever` form. The focused
cycle/version population passes **112/112** and the complete registered scan
passes **17/17 rules / 122 controls**, including **66** R12 controls.

The stable ledger's dated Rust 1.86/1.85 observation initially exposed a prior
LEDGER omission: `version-check` correctly rejected `docs/DEFERRED.md` as an
unclassified floor-literal file. The file-level partition now places that dated
ledger in its historical-family class. The rerun passes over **596 tracked
files**, **101 literal-bearing files**, and **1,047 literal occurrences**, with
**0** classification decisions. This forward-corrects the earlier claim rather
than treating the first self-test failure as a clean result.

The permission-capable full local matrix passes **22/22**; its Python 3.11
population passes **400/400**, direct R17 readback agrees across **30 refs / 1
audit**, and all default, net, Rust-floor, clippy, format, protected-artifact,
and fingerprint lanes pass. The first sandboxed matrix reached the net test but
could not bind its loopback server (`Operation not permitted`); the identical
permission-capable rerun passes all **29/29** ingest net tests and **30/30**
cored net tests. The independently executed Python 3.12 population passes
**400/400**. Embedded and standalone golden each pass **11/11** with zero delta.
Before this record, State measured **229,439 / 453,741 bytes**; the protected
manifest remains **200,440 / 1,048,576 bytes**. No dependency, production
runtime source, public value domain, protected byte, publisher wire, tag,
branch, or remote ref moved. The three protected untracked amendment inputs
remain untouched, and no DOC-BOUNDARY gate or stop condition fired.

**v0.43 LEDGER — canonical deferral authority established (measured
2026-08-05).** `docs/DEFERRED.md` is now the sole live 31-row deferral table.
It measures **10,367 bytes**, names v0.43 exactly once, and lives outside
`docs/cycles/`, so the review-retention partition neither duplicates nor drops
it. The active runbook and progress record contain no semantic deferral-table
heading. C32 selects the ledger at the active runbook's activation parent as
the standing comparison. The one-time migration fallback reads v0.42 from
activation parent `6f07edf84c3ce40f1ef4c9e97e5d101242490243`; exact per-cell
comparison reports **31 subjects / 0 subject differences / 0 trigger
differences**. This comparison reads Git/worktree documents directly and does
not consume a Repomix export or its retained membership.

R12 now carries **65** fail-before controls and the complete registry carries
**17 rules / 121 controls**. Its first new control rejects a stale or omitted
ledger-cycle advance; the other two reject a missing carried subject and a
byte-different trigger. The complete mutation self-test passes **17/17**, and
the focused cycle-check population passes **98/98**. The permission-capable
local matrix passes **22/22** with direct R17 readback at **30 refs / 1 audit**,
clean Rust default/net/MSRV/lint/format lanes, and Python 3.11 at **399/399**.
The separately executed Python 3.12 population also passes **399/399**; both
carry only the accepted non-fatal warning. Embedded and standalone golden runs
each pass **11/11**.

Grant H was not issued. Because migration completed, Step 2b is
`not-granted`, not `not-applicable`; the derived review-retention depth remains
**2 cycles** and the configured set remains v0.42–v0.43. `cycle-check` reports
the two facts independently as `retention_depth=2` and
`retention_change=not-granted`. No dependency, production runtime source,
protected byte, observation, fixture, publisher wire, entitlement/licensing
outcome, local tag, branch, or remote ref moved. The three pre-existing
untracked amendment inputs remain untouched.
