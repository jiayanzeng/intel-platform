...
**As of:** 2026-08-03 · **Version:** v0.17.2 (core-shell) · **Status:** **v0.35 R-CLOSE is preparing the operator-selected patch release locally; publication remains unauthorized.** Authenticated run `30762871542` passed all **9** blocking identities, the pinned release-grade verifier accepted **9/9** signed bundles, and asserted receipts bind the Rust **1.78.0 / 1.86.0 / 1.85.0** floor lanes. The patch corrects a v0.17.1 verifier that cannot validate a bundle under pinned `gh` 2.96.0, qualifies and independently checks certificate identity, makes every floor lane select and prove its effective toolchain, and binds that release into each receipt. No route, response-body schema, named-surface shape, dependency resolution, protected database, golden input, entitlement/licensing outcome, or serialized `/v1/*` value domain moves. The registered suite remains **12 rules / 73 controls** with **0** hand-typed absolute finding-line fields; the retraction count remains **3**. Published `main` and v0.17.1 are unchanged until a separate exact publication authorization; the four v0.35 evidence refs and their runs remain immutable.
