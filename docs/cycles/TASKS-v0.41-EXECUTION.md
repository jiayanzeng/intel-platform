...
| Second `STATE.md` archival | the review-export attention predicate fires, or `STATE.md` reaches its governed artifact byte boundary | v0.41 · 2026-08-04 — ARCHIVE evaluated the exact grant text and records **not-granted**: publication authority did not issue Grant E. State is **185054 / 453,741 bytes**. Governed review-export **2,789,050** and the latest measured REVIEW-SOURCE export **2,638,599** both meet the **2,569,388-byte** attention boundary while the State boundary does not, so the repaired attention clause fires. trigger-fired disposition: kind=`unheld-lever`; lever=`Grant E`; recoverable_bytes=`84896`. The exact v0.38 archival lever remains unexecuted and unrecovered; no structural archive or manifest pin was created. | Step 2 and Step 5 |
