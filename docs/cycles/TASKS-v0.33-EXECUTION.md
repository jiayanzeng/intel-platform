...
|---|---|---|---|
| T7 robots single-flight | a second concurrent harvester | v0.33 · 2026-08-01 — Step 6's isolated local and hosted test runtimes started no harvester, made no publisher request, and never created a second concurrent harvest caller; trigger did not fire | none |
...
| Live multi-publisher behaviour in one runtime | further origins beyond the two configured, or concurrency | v0.33 · 2026-08-01 — Step 6 exercised only fixture/test runtimes: no live publisher runtime, third origin, or concurrency was exercised; trigger did not fire | none — complete, do not re-exercise |
...
| R3/R4 open-bottom coverage | a spelling outside registered vocabulary | v0.33 · 2026-08-01 — the exact candidate's registered 12-rule / 67-control vocabulary passed locally and no new spelling appeared; trigger did not fire | none |
...
| Manifest retention/indexing | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take ≥1.00 s real | v0.33 · 2026-08-01 — Step 6 reverified the unchanged 192,042-byte / 332-pin manifest at the exact authenticated candidate, locally matching every pin and both protected databases while hosted `core` passed schema validation; no registration occurred. The latest complete timed pair remains Step 5's 0.11 s / 0.10 s real, so neither trigger fired | Step 5 — completed with the exact through-v0.28 structural archive pin |
