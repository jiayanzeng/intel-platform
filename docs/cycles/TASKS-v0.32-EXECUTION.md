...
| Second `STATE.md` archival | the export ceiling trigger fires, or `STATE.md` reaches its governed artifact byte boundary | v0.32 · 2026-08-01 — the lifecycle checker read 339,465 bytes directly and reported `bound`, leaving 114,276 bytes; at the latest +35,173-byte/cycle denominator that is 3.25 cycles, down from v0.31 activation's 5.19-cycle estimate. This remains the nearest governed boundary; neither clause fired | Step 2 — measured directly; boundary made executable; no archive |
| Planted-control line numbers re-derived by hand | a control-schema change, or a cycle in which the re-derived count exceeds the controls it protects | v0.32 · 2026-08-01 — Step 2 and Step 3 each added one R12 control, so the control-schema trigger fired twice; all 12 shifted existing `expected_line` values (nine in Step 2 and three in Step 3) were re-derived from real self-test output and remain fewer than the 60 controls protected | Step 2, Step 3, Step 4, and Step 5 — shifted values re-derived and counted |
