...
| public `/v1/*` response-domain stability | shell Pydantic serialization authorities + dependency-free source derivation + runtime OpenAPI equivalence + release baseline + R15 | before third-party installation, every declared success/error status and media schema on the six public routes is recursively reduced from the actual decorators and type annotations to fields, requiredness, container/primitive types, nullability, object openness, and literal values; the installed shell population independently requires that source result to equal FastAPI's OpenAPI output, including the pinned native 422 domain; R15 diffs the derivation against the release baseline, while planted mutations prove that an added enum value, removed field, and changed field type are each rejected at their serialization control site |
...
project root and derives its required review set from the Git-tracked tree. The
only tracked subtractions are the derived retained-cycle complement, the
nonempty pinned-observation `binary` raw-wire class, manifest-derived structural
archive roots, the configured bulk-evidence root derived from pinned paths, and
the mixed-use protected-manifest class derived from an exact projection carried
by a tracked review-visible Markdown file; there is no hand-maintained
required-path list. The projection source path comes from its tracked marker,
not from a checker list. It retains every non-pin manifest field and exactly the
pins whose referenced paths survive the earlier review-source derivation. A
source qualifies only when both visible and non-visible pins exist. Every path
that actually lands in the export is independently required to be Git-tracked.
Exact configured observation exclusions must equal the derived raw-wire class
in both directions, and exact configured pinned-manifest exclusions must equal
the nonempty projected-manifest class in both directions. The projection's
deterministic JSON must equal its manifest derivation byte-for-byte, so neither
a newly captured raw body, a review-source file, nor a stale manifest head can
cross the seam silently. The manifest remains the tracked integrity authority;
the projection changes review carriage only. `AGENTS.md` records the two older
measured operating failures:
non-root generation silently dropped `Cargo.lock`, and Repomix's security pass
included 339 of 340 collected files while omitting
...
A governed trigger disposition is substantive only in one of two exact,
machine-readable forms. `kind=measured-change` names a subject plus distinct
baseline and current byte quantities. `kind=unheld-lever` names the unavailable
lever plus a positive recoverable-byte quantity. Exactly one disposition prefix
and exactly one valid form must occur in the governed measured cell, so a weak
awareness sentence cannot be hidden beside a valid-looking suffix. Export
attention and governed artifact-trigger crossings consume this same parser.
The unheld-lever form is deliberately satisfiable before authorization exists:
it records the truthful remaining option and quantity without claiming it ran;
a later measurement can test whether that lever landed or replace the answer
with measured change.
...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export meets or exceeds the executable two-governed-growth-cycle attention boundary, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, any exported path is not Git-tracked, the exact configured observation exclusions differ from the nonempty derived raw-wire class, any derived raw-wire body reappears, or any manifest-derived structural-archive byte reappears | v0.43 · 2026-08-05 — ACTIVATE retains ATTENTION-BASIS's maximum positive adjacent governed delta, v0.34 **2,527,180 →** v0.35 **2,742,486 (+215,306)**. Review-export attention boundary: headroom_cycles=`2`; denominator_bytes_per_cycle=`215306`; boundary_bytes=`2569388`. The accepted **3,000,000-byte ceiling and two-cycle retention depth are unchanged**. The latest governed measurement remains exact v0.42 release parent `5452355945d2717cbd84ea2224148dbd0f4c1ac7` with a project-root export of **2,710,728 bytes** across **157 Git-tracked files / 2 retained cycles**, **141,340 bytes above** attention and **289,272 bytes / 9.64% / 1.34 high-water cycles** below the ceiling. trigger-fired disposition: kind=`measured-change`; subject=`review-source export`; baseline_bytes=`2657685`; current_bytes=`2710728`. Grants E and H remain unissued and contribute **0 bytes**; their measured levers remain **84,896** and **88,665 bytes** respectively. The margin continues to share the high-water denominator: Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.34.md`; prior_bytes=`2527180`; current_progress=`docs/cycles/PROGRESS-v0.35.md`; current_bytes=`2742486`; evaluated_progress=`docs/cycles/PROGRESS-v0.42.md`; evaluated_bytes=`2710728`; denominator_bytes_per_cycle=`215306`; numerator_bytes=`289272`; cycles=`1.34`. A later adjacent governed increase above **215,306** falsifies and replaces this numeric basis; a quiet, decrease, or smaller increase cannot shrink the reserve. Governed review-export bytes: `2710728`. |
| retained cycle-document set (v0.43) | **Accepted with a one-high-water-cycle reserve inside review-export failure — 2026-08-05** | the retained cycle-document raw-byte set reaches its governed boundary | v0.43 · 2026-08-05 — DOC-BOUNDARY governs the exact active-plus-prior runbook/progress set because those are the cycle documents that enter the review export. Exact baseline tree `f8407acaf3fdc3aa4b330950217489f2c117cb62` exports **2,726,105 bytes**; its four retained cycle-document payloads contribute **139,364 bytes**, leaving a measured **2,586,741-byte non-cycle payload**. Preserving the maximum positive adjacent governed export delta of **215,306 bytes** between this contributor's boundary and the unchanged **3,000,000-byte ceiling** derives **3,000,000 - 2,586,741 - 215,306 = 197,953 bytes**. Raw repository bytes are the conservative set measure: the baseline was **139,368 bytes / 4 paths**, and the completed DOC-BOUNDARY worktree measures **140,818 bytes / 4 paths**, leaving **57,135 bytes / 28.86%** to the boundary. At the boundary, holding the measured non-cycle baseline fixed, the combined projection is **2,784,694 bytes** and keeps the full **215,306-byte** reserve. A later larger high-water delta lowers this boundary; a quiet or smaller delta cannot shrink the reserve. The independent whole-export attention control covers non-cycle growth. Cycle-document boundary: set=`retained`; baseline_tree=`f8407acaf3fdc3aa4b330950217489f2c117cb62`; baseline_export_bytes=`2726105`; baseline_cycle_content_bytes=`139364`; baseline_non_cycle_bytes=`2586741`; reserve_bytes=`215306`; boundary_bytes=`197953`. The trigger did not fire. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.43 · 2026-08-05 — ACTIVATE validates schema v2 at **3 artifacts / 339 pinned files** and measures the manifest at **200,440 bytes**, leaving **848,136 bytes** to its governed boundary. The latest two consecutive complete checks matched both structural archives, all other pins, and all three protected databases in **0.10 s / 0.10 s real**, leaving **0.90 s / 0.90 s** to the timing trigger. Neither boundary nor timing trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.43 · 2026-08-05 — ACTIVATE changes no dependency declaration or constraint. Permission-capable Python 3.11 and Python 3.12 populations each pass **397/397** with the warning non-fatal. Neither warning trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.43 · 2026-08-05 — Permission-capable R17 readback gives published `main=993813c755e9f759a4ee165954c7a1df984f6b10`; pre-disposition HEAD `876cd9a561d81965b48449ed2b4f07bed438ed47` is its 41-first-parent-commit descendant. The distance changes lifecycle/checker controls, governing records, and release-version authorities only. R15 derives **0 public response-domain differences** across **6 routes / 31 status-media variants / 112 field occurrences**. No measured runtime behavior or public surface moves and the current publication-epoch count remains **0**; neither trigger clause fires. Grant G was not issued, so no remote ref moved. |
...
body shape stay fixed. R15 makes the domain comparison executable against the
release-baselined manifest; R-CLOSE still owns the release-class adjudication
and must cite that manifest diff rather than substitute prose inspection.
