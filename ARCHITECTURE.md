...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.36 · 2026-08-03 — Step 0 measured the project-root worktree export at **2,724,915 bytes / 152 files / 2 retained cycles**, with exactly the v0.35–v0.36 task/progress pairs retained and both protected byte classes excluded. The latest closed, progress-backed governed figure remains an export of **2,742,486 bytes**, so the open-cycle content-binding exemption keeps the prior governed series **2,527,180 → 2,742,486 (+215,306)** until a v0.36 implementation commit and progress field exist. Its governed **257,514-byte / 8.58%** remainder is **1.20 cycles** at that executable denominator. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.34.md`; prior_bytes=`2527180`; current_progress=`docs/cycles/PROGRESS-v0.35.md`; current_bytes=`2742486`; evaluated_progress=`docs/cycles/PROGRESS-v0.35.md`; evaluated_bytes=`2742486`; denominator_bytes_per_cycle=`215306`; numerator_bytes=`257514`; cycles=`1.20`. The current worktree measurement independently leaves 275,085 bytes / 9.17%, but does not replace the progress-bound figure. Neither byte nor retention trigger fired. Governed review-export bytes: `2742486`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.36 · 2026-08-03 — Step 0 measured the unchanged manifest at **193,057 bytes / 332 pins**, leaving **855,519 bytes** to its governed boundary. Two complete checks matched every pin, the State archive, and both protected databases in **0.12 s / 0.13 s real**, leaving **0.88 s / 0.87 s** to the timing trigger. No manifest or protected byte changed and neither trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.36 · 2026-08-03 — Step 0's sandboxed Python 3.11 population collected **366**, passed **357**, and failed **9**: eight failures were measured environment denials of loopback binding or process inspection, and one was the expected stale v0.35 trigger-observation failure corrected by the active-cycle entries. The same run emitted the existing single warning. The permission-complete `ci-local` acceptance then stopped earlier at active-cycle consistency, before reaching the shell lane. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed; neither warning trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.36 · 2026-08-03 — Activation and Step 0 change lifecycle documentation and governance controls only. No production source, route, response-body schema, named public-surface shape, or serialized `/v1/*` value domain changed. Published v0.17.1's epoch count remains **0**; locally closed, unpublished v0.17.2 still carries no measured runtime-behaviour difference. Neither trigger fired. |
