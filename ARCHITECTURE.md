...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.30 · 2026-07-31 — The activation audit tree `e7b2c58814e2223d9899b83b3f3491344ce85337` produced an exact project-root export of **2,464,445 bytes / 152 files**, leaving **535,555 bytes / 17.85%** headroom. `export_check` retained exactly the v0.28, v0.29, and v0.30 TASKS/PROGRESS pairs, rejected both excluded byte classes, and reported **99 derived / 7 required** paths. This supersedes v0.29's 2,516,822-byte close-time observation; both measurements remain in their dated records. Neither trigger fired. Governed review-export bytes: `2464445`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest first reaches **1 MiB**, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.30 · 2026-07-31 — Activation validation measured **331** pins and an actual **191,395-byte** manifest, leaving **857,181 bytes** to 1 MiB; two consecutive complete verifications took **0.09 s / 0.10 s real**, leaving **0.91 s / 0.90 s** to the timing trigger, and matched both protected databases. Neither trigger fired. The manifest and every protected byte remain unchanged from v0.29 close. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.30 · 2026-07-31 — Clean-rebuilt Python 3.11.4 and 3.12.13 each collected **306**, passed **306**, failed **0**, and skipped **0**, with the same one warning. `tools/test_population.py` derived `collected=306`, `equivalent=true`, and `equivalent_passed=306`. No dependency declaration or constraint changed, so neither trigger fired. The first 3.11 activation attempt's expected governed-row identity failure remains recorded in E0 rather than being presented as a passing result. |
