...
| public `/v1/*` response-domain stability | shell Pydantic serialization authorities + release-baselined derived manifest + R15 | every declared success/error status and media schema on the six public routes is recursively reduced to fields, requiredness, container/primitive types, nullability, object openness, and literal values; R15 diffs that derivation against the release baseline, while planted mutations prove that an added enum value, removed field, and changed field type are each rejected at their serialization control site |
