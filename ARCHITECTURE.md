...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.37 · 2026-08-03 — R-CLOSE's exact release parent produced an export of **2,674,055 bytes** across **158 files / 2 retained cycles**, with exactly the v0.36–v0.37 task/progress pairs retained and both protected byte classes excluded. It leaves **325,945 bytes / 10.86% / 2.27 cycles** at the latest +143,456-byte adjacent-cycle denominator. STATE-ARCHIVE's earlier delivered structural baseline passed at **2,558,258 bytes**, **41,742 bytes** below C10's 2,600,000-byte target; subsequent DOMAIN-MANIFEST, evidence, and close records put the release parent **74,055 bytes** above that step target without crossing the architectural ceiling. The latest positive distinct-cycle progress-backed governed denominator remains **2,742,486 → 2,885,942 (+143,456)** across v0.35→v0.36; the v0.37 structural reclaim is an evaluated baseline, not a positive growth term. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.35.md`; prior_bytes=`2742486`; current_progress=`docs/cycles/PROGRESS-v0.36.md`; current_bytes=`2885942`; evaluated_progress=`docs/cycles/PROGRESS-v0.37.md`; evaluated_bytes=`2674055`; denominator_bytes_per_cycle=`143456`; numerator_bytes=`325945`; cycles=`2.27`. Neither byte, retention, nor protected-content trigger fired. Governed review-export bytes: `2674055`. |
