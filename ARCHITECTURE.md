...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.35 · 2026-08-03 — Step 7 measured exact untagged release parent `d4258883645a99f9499895bf064e453de9be1281` at an export of **2,742,486 bytes / 152 files / 2 retained cycles**, with the v0.34–v0.35 task/progress pairs retained and both protected byte classes excluded. The closing child binds to that already-existing release-parent commit; v0.34 confirms that the machine field named `tree=` stores the commit object, not its distinct Git tree object. The closing-visible governed series now ends **2,527,180 → 2,742,486 (+215,306)** across the latest adjacent cycle pair. The **257,514-byte / 8.58%** remainder is **1.20 cycles** at that executable denominator, making export the nearest governed byte boundary. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.34.md`; prior_bytes=`2527180`; current_progress=`docs/cycles/PROGRESS-v0.35.md`; current_bytes=`2742486`; evaluated_progress=`docs/cycles/PROGRESS-v0.35.md`; evaluated_bytes=`2742486`; denominator_bytes_per_cycle=`215306`; numerator_bytes=`257514`; cycles=`1.20`. The entry point still states that one adjacent pair has no representativeness guarantee and cannot detect a structural epoch. Neither byte nor retention trigger fired. Governed review-export bytes: `2742486`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.35 · 2026-08-03 — Step 7 reverified exact release parent `d4258883645a99f9499895bf064e453de9be1281` at **193,057 bytes / 332 pins**, **+1,015 bytes** from v0.34 and leaving **855,519 bytes / 842.88 cycles** at that adjacent-cycle denominator. Two complete checks matched every pin, the State archive, and both protected databases in **0.11 s / 0.10 s real**, leaving **0.89 s / 0.90 s** to the timing trigger. No manifest or protected byte changed during Step 7 and neither trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.35 · 2026-08-03 — Step 7 changes release literals and closing records only. The latest exact-candidate local Python 3.11.4 and 3.12.13 populations each passed **366/366** with the same one warning, and their hosted comparators each derived **366** equivalent passed. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed; the warning remains a warning and neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.35 · 2026-08-03 — Step 7 classifies the v0.17.2 release distance as verifier/toolchain evidence correction plus release literals and lifecycle records, with no measured runtime-behaviour difference, route, response-body schema, named-surface shape, or serialized `/v1/*` value-domain change. Published v0.17.1's epoch count remains **0**. The local tagged close publishes nothing; a separately authorized v0.17.2 publication will reset and retain the count at **0** on its closing commit. |
