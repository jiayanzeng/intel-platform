...
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.33 · 2026-08-01 — Operator-selected Fidelity B added one exact structural archive pin. Step 5 measured the manifest at **192,042 bytes / 332 pins**, reported `bound`, and matched every pin plus both protected databases in two complete runs taking **0.11 s / 0.10 s real**, leaving **0.89 s / 0.90 s** to the timing trigger. The latest same-kind positive delta is **647 bytes/cycle** and the remaining **856,534-byte** margin is **1,323.85 cycles** at that denominator. Neither trigger fired. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.33 · 2026-08-01 — Step 5 local Python 3.11.4 and 3.12.13 lanes each collected/passed **346**, failed **0**, and skipped **0**, with the same one accepted warning. `tools/test_population.py` derived `collected=346`, `equivalent=true`, and `equivalent_passed=346` for the two local lanes. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and no warning became an error. Neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.33 · 2026-08-01 — Direct E0 diff classification found that the v0.33 activation and gate-measurement distance contains only lifecycle documentation, retained-cycle configuration, and cycle records: no production source, workflow, manifest, dependency, release value, runtime-behaviour, or public-surface change. Published v0.17.1 reset the count to zero at closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`; the count remains zero and no fresh count has started. Direct remote inspection again resolved `main`, peeled `v0.17.1`, and its annotated object unchanged. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
