...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.35 · 2026-08-02 — Step 1 ran the project-root export at exact delivered v0.34 audit child `d8d20b81b9ea9027dada74ce047a7cd92815e9f3` and measured **2,559,695 bytes / 151 files / 2 retained cycles**, confirming the reviewer figure and tree identity with both protected byte classes excluded. The closing-visible governed series remains **2,576,273 → 2,629,379 (+53,106) → 2,706,393 (+77,014) → 2,592,441 (−113,952) → 2,527,180 (−65,261)**; no adjacent same-kind pair exists yet wholly inside the two-cycle-retention epoch, so Step 4 owns the next basis. The live machine row therefore remains bound to exact authenticated v0.34 candidate `1117dc6db6ec0e55e8c8f078ca8059628f9f8262` at an export of **2,527,180 bytes / 151 files / 2 retained cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; evaluated_progress=`docs/cycles/PROGRESS-v0.34.md`; evaluated_bytes=`2527180`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`472820`; cycles=`6.14`. That basis and the published 77,862, 86,946, and 79,962-byte denominators all predate the v0.34 retention change and are epoch-stale for selecting a new lever. Neither byte trigger fired. Governed review-export bytes: `2527180`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.35 · 2026-08-03 — Step 6 reverified exact authenticated candidate `2e5b247e348f362b1cc3fa6a9aaa393d0025fc87` at **193,057 bytes / 332 pins**, **+1,015 bytes** from v0.34 and leaving **855,519 bytes / 842.88 cycles** at that one-pair denominator. The only changed manifest entry is the existing `run` authorization pin/provenance; `tools/model_profiles.py`, all evidence pins, both protected database admissions, and every protected byte remain unchanged. Two complete candidate runs matched all pins, the exact structural State archive, and both protected databases in **0.13 s / 0.11 s real**, leaving **0.87 s / 0.89 s** to the timing trigger. Hosted `core` independently passed schema validation. Neither trigger fired; wall-clock timing remains outside the lifecycle byte rule. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.35 · 2026-08-03 — Step 6 exact-candidate local Python 3.11.4 and 3.12.13 each collected/passed **366**, failed **0**, skipped **0**, and emitted the same one warning. Each hosted lane collected **366**, passed **365**, and skipped the same named, reasoned `on_site` test; for each local/hosted pair `tools/test_population.py` derived `collected=366`, `equivalent=true`, and `equivalent_passed=366`. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed, the warning remained a warning rather than an error, and neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.35 · 2026-08-03 — Step 6 classified exact authenticated candidate `2e5b247e348f362b1cc3fa6a9aaa393d0025fc87` against published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. Its distance contains lifecycle documentation/tests/controls, the two-sided net-floor workflow, verifier and launcher tooling, and one updated `run` authorization pin, but no production source, dependency resolution, release value, measured runtime-behaviour difference, route, response body, or public value-domain change. Published v0.17.1 reset the count to zero and no fresh count has started. The sole remote changes were the authorized fresh evidence ref and its one hosted run; direct readback left remote `main`, peeled v0.17.1, and the annotated tag object unchanged. |
