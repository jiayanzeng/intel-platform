...
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.35 · 2026-08-02 — Step 5 changed exactly the existing `run` pin after adding the paired net-floor lanes. The manifest is now **192,370 bytes / 332 pins**, a **+328-byte** same-kind delta, leaving **856,206 bytes / 2,610.38 cycles** at that one-pair denominator. Schema validation and complete artifact verification matched every pin, the exact structural State archive, and both protected databases; `tools/model_profiles.py` remains byte-identical. The latest explicitly timed pair remains Step 1's **0.11 s / 0.10 s real**, below the timing trigger. Neither trigger fired; wall-clock timing remains outside the lifecycle byte rule. |
...
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.35 · 2026-08-02 — Through Step 5, the v0.35 distance over published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04` contains lifecycle controls and tests, cycle/architecture records, the two-sided net-floor workflow and launcher lanes, and one updated `run` authorization pin. It contains no production source, dependency resolution, release value, measured runtime-behaviour difference, route, response body, or public value-domain change. Published v0.17.1 reset the count to zero and no fresh count has started. No publisher request or remote ref mutation occurred. |
