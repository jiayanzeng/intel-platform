...
archive roots, and the configured bulk-evidence root derived from pinned paths;
there is no hand-maintained required-path list. Every path that actually lands
in the export is independently required to be Git-tracked. Exact configured
observation exclusions must equal the derived raw-wire class in both directions,
so neither a newly captured raw body nor a review-source file can cross the seam
silently. `AGENTS.md` records the two older measured operating failures:
