...
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.39 · 2026-08-04 — E0's complete Python 3.11 and Python 3.12 populations each pass **373/373** with the warning non-fatal. The entering tree through E0 changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. Neither warning trigger fired. |
