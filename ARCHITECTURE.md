...
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.33 · 2026-08-01 — E0 read the unchanged **191,395-byte** manifest, reported `bound`, re-validated all **331** pins, and matched both protected databases in two complete runs taking **0.09 s / 0.10 s real**, leaving **0.91 s / 0.90 s** to the timing trigger. The latest cycle delta remains zero; at the last positive **8,621-byte/cycle** denominator the byte margin remains **99.43 cycles**. Neither trigger fired. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.33 · 2026-08-01 — Step 3 local Python 3.11.4 and 3.12.13 lanes each collected/passed **342**, failed **0**, and skipped **0**, with the same one accepted warning. `tools/test_population.py` derived `collected=342`, `equivalent=true`, and `equivalent_passed=342` for the two local lanes. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and no warning became an error. Neither trigger fired. |
