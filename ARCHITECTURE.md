...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.34 · 2026-08-02 — Step 5 implemented operator-selected Option A after exact construction measurement. Baseline tree `8fe4225fb7c53c8d146fb5a0725bdc19983d16de` exported **2,629,024 bytes / 153 files / 3 retained cycles**; construction tree `c19876d08502a8aa4eb33e35d25ce2b7d67f32e5` exported **2,520,904 bytes / 151 files / 2 retained cycles**, an exact **108,120-byte** reclaim. At the unchanged 77,862-byte post-retention steady denominator, margin moved **4.76 → 6.15 cycles (+1.39)**. The executable depth authority and Repomix pattern now retain the active cycle plus one prior. The latest closing-visible governed evaluation point remains authenticated candidate `2edb7694c2c6c1498b3903382c37aef68329150d` at an export of **2,592,441 bytes / 153 files**, leaving **407,559 bytes / 13.59%**. The full last-per-progress governed series remains **2,576,273 → 2,629,379 (+53,106) → 2,706,393 (+77,014) → 2,592,441 (−113,952)**. The selected latest positive basis still predates the v0.33 State archival; the checker enforces identity and arithmetic but has neither an epoch signal nor a representativeness floor. On that basis the governed remainder is **5.29 cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; evaluated_progress=`docs/cycles/PROGRESS-v0.33.md`; evaluated_bytes=`2592441`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`407559`; cycles=`5.29`. The 3,000,000-byte ceiling remains an executable repository proxy for a project-knowledge capacity problem last observed externally in v0.28 as **2,067 chunks / 2,000 limit** on a 4,975,987-byte export; the repository has no chunker or current project-knowledge index and cannot recalibrate the proxy against that referent. Neither byte trigger fired, and both protected byte classes remain excluded. Governed review-export bytes: `2592441`. |
...
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.34 · 2026-08-02 — Step 6 classified exact authenticated candidate `1117dc6db6ec0e55e8c8f078ca8059628f9f8262` against published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. Its file list contains lifecycle documentation, tests, controls, and retained-cycle configuration, but no production source, workflow, dependency, release value, measured runtime-behaviour difference, or public-surface change. Published v0.17.1 reset the count to zero at that closing commit; the count remains zero and no fresh count has started. Final direct remote inspection left `main`, peeled `v0.17.1`, and annotated object `14912f134e45277e2b4fd10b7f5bf8b4900ca20d` unchanged. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
