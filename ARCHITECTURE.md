...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.32 · 2026-08-01 — Exact no-release evidence candidate `1caace6e2470b51c371d67598b756a48f93d7968` produced an operator-local project-root export of **2,706,393 bytes / 153 files**, leaving **293,607 bytes / 9.79%** headroom. `export_check` retained exactly the v0.30, v0.31, and v0.32 TASKS/PROGRESS pairs, rejected both excluded byte classes, and reported **100 derived / 7 required** paths. This is **51,989 bytes larger** than v0.31's delivered export; at that delivered-growth denominator the remaining margin is **5.65 cycles**. The export trigger did not fire. Governed review-export bytes: `2706393`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.32 · 2026-08-01 — Close-time validation read the actual **191,395-byte** manifest, reported `bound`, re-validated all **331** pins, and matched both protected databases in two complete runs taking **0.19 s / 0.11 s real**, leaving **0.81 s / 0.89 s** to the timing trigger. The latest cycle delta is zero; at the last positive **8,621-byte/cycle** denominator the byte margin is **99.43 cycles**. Neither trigger fired. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.32 · 2026-08-01 — Exact-candidate local Python 3.11.4 and 3.12.13 lanes each collected/passed **336**, failed **0**, and skipped **0**, with the same one accepted warning. Hosted run 30693555131 collected **336** in each lane, passed **335**, and skipped the same one named, reasoned `on_site` node; `tools/test_population.py` independently derived `collected=336`, `equivalent=true`, and `equivalent_passed=336` for both local/hosted pairs. No dependency declaration or constraint changed from the published v0.17.1 closing tree, so neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.32 · 2026-08-01 — The trigger fired at v0.31 when the bounded internal `/ingest` diagnostic correction reached its third consecutive unpublished close, and v0.17.1 publication disposed it and reset the consecutive count to zero at closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. The operator selected `no-release` after the close-time diff confirmed that v0.32 contains only documentation, lifecycle checks, focused control tests, and cycle records: no production source, workflow, manifest, dependency, release-value, runtime-behaviour, or public-surface change. Close-time remote inspection again resolved `main` and peeled `v0.17.1` to the published closing commit with its annotated tag object unchanged. The publication-epoch count therefore remains zero and no fresh count has started. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
