...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.38 · 2026-08-04 — Exact release parent `37f552c0c326098bdcf8f19de7eac19670d74680` produced an export of **2,781,281 bytes** across **163 files / 2 retained cycles**, with exactly the v0.37–v0.38 task/progress pairs retained, both protected raw RSS bodies excluded, and all `docs/state-archive/**` bytes excluded. It leaves **218,719 bytes / 7.29% / 2.04 cycles** at the latest +107,226-byte adjacent-cycle denominator. The latest positive adjacent distinct-cycle progress-backed governed denominator is **2,674,055 → 2,781,281 (+107,226)** across v0.37→v0.38. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.37.md`; prior_bytes=`2674055`; current_progress=`docs/cycles/PROGRESS-v0.38.md`; current_bytes=`2781281`; evaluated_progress=`docs/cycles/PROGRESS-v0.38.md`; evaluated_bytes=`2781281`; denominator_bytes_per_cycle=`107226`; numerator_bytes=`218719`; cycles=`2.04`. Neither byte, retention, nor protected-content trigger fired. Governed review-export bytes: `2781281`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.38 · 2026-08-04 — WIRE-ADMISSION and the release-parent gate validated schema v2 at **3 artifacts / 339 pinned files** and measured the manifest at **200,440 bytes**, leaving **848,136 bytes** to its governed boundary. The latest consecutive complete checks matched both structural archives, all other pins, and all three protected databases in **0.09 s / 0.09 s real**, leaving **0.91 s / 0.91 s** to the timing trigger. Neither boundary nor timing trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.38 · 2026-08-04 — The v0.17.5 release-parent full gate passed Python 3.11 and Python 3.12 at **373/373** each with the warning remaining non-fatal. v0.38 changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. Neither warning trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.38 · 2026-08-04 — Grant A published exact v0.17.4 closing commit `f4f2690a442d7a77f1dabb53fb3a120a2c987e97` and reset the consecutive unpublished-divergence count to **0** there. v0.38 is the first subsequent closed cycle with a measured runtime-behavior difference: SEC admission enables bounded harvesting of the already-configured source and adds the intended finance corpus. R15 reports **0 public response-domain differences** across **6 routes / 31 status-media variants / 112 field occurrences**, so there is no public-surface change and the count becomes **1**. Neither the three-cycle nor immediate trigger fires; the unpublished local close does not reset the epoch. |
