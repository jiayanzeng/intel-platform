...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export meets or exceeds the executable two-governed-growth-cycle attention boundary, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, any exported path is not Git-tracked, the exact configured observation exclusions differ from the nonempty derived raw-wire class, any derived raw-wire body reappears, or any manifest-derived structural-archive byte reappears | v0.41 · 2026-08-04 — ACTIVATE retains ATTENTION-BASIS's maximum positive adjacent governed delta, v0.34 **2,527,180 →** v0.35 **2,742,486 (+215,306)**. Review-export attention boundary: headroom_cycles=`2`; denominator_bytes_per_cycle=`215306`; boundary_bytes=`2569388`. The reserve remains **430,612 bytes** and the accepted **3,000,000-byte ceiling is unchanged**. REVIEW-SOURCE derives one mixed-use manifest from a tracked exact projection, retaining every non-pin field and exactly **9/339** review-visible pins while omitting **330** unusable pin records. The resulting exact worktree export is **2,638,599 bytes / 157 Git-tracked files / 2 retained cycles**, **69,211 bytes above** the attention boundary and **361,401 bytes / 12.05% / 1.68 high-water cycles** below the fixed ceiling. trigger-fired disposition: kind=`measured-change`; subject=`review-source export`; baseline_bytes=`2803926`; current_bytes=`2638599`. The exhaustive current partition is **158 tracked repository paths / 157 required paths present / 0 untracked defects / 4 derived raw-wire exclusions absent / 2 structural archives absent / 1 derived review-manifest exclusion absent**; the protected manifest remains byte-identical and authoritative, and the three historical amendment inputs remain untouched and excluded. The margin continues to share the high-water denominator: Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.34.md`; prior_bytes=`2527180`; current_progress=`docs/cycles/PROGRESS-v0.35.md`; current_bytes=`2742486`; evaluated_progress=`docs/cycles/PROGRESS-v0.40.md`; evaluated_bytes=`2789050`; denominator_bytes_per_cycle=`215306`; numerator_bytes=`210950`; cycles=`0.98`. A later adjacent governed increase above **215,306** falsifies and replaces this numeric basis; a quiet or smaller increase cannot shrink the reserve. The currently bound export of **2,789,050 bytes** remains the v0.40 release-parent measurement. Governed review-export bytes: `2789050`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.41 · 2026-08-04 — ACTIVATE validates schema v2 at **3 artifacts / 339 pinned files** and measures the manifest at **200,440 bytes**, leaving **848,136 bytes** to its governed boundary. Two complete checks matched both structural archives, all other pins, and all three protected databases in **0.10 s / 0.10 s real**, leaving **0.90 s / 0.90 s** to the timing trigger. Neither boundary nor timing trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.41 · 2026-08-04 — ACTIVATE changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. The latest complete Python 3.11 and Python 3.12 populations remain the measured v0.40 **391/391** passes with the warning non-fatal. Neither warning trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.41 · 2026-08-04 — Published v0.17.5 keeps the publication epoch at **0**; permanently withheld v0.17.6 never resets it, and unpublished v0.17.7 carries no measured runtime difference. ACTIVATE adds no runtime behavior or public surface, and fresh R15 derivation reports **0 public response-domain differences** across **6 routes / 112 field occurrences**. The count remains **0** and neither trigger clause fired. |
