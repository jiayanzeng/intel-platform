...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.31 · 2026-07-31 — The activation audit tree `9ed9f9e8086f703d9d349878e6fe14320e5e7b9d` produced an exact project-root export of **2,544,715 bytes / 153 files**, leaving **455,285 bytes / 15.18%** headroom. `export_check` retained exactly the v0.29, v0.30, and v0.31 TASKS/PROGRESS pairs, rejected both excluded byte classes, and reported **100 derived / 7 required** paths. The v0.30 delivered-to-v0.31-activation movement is **−38,909 bytes**; retention removed the v0.28 pair while the v0.31 pair entered scope. Neither trigger fired. Governed review-export bytes: `2544715`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest first reaches **1 MiB**, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.31 · 2026-07-31 — Activation validation measured **331** pins and an actual **191,395-byte** manifest, leaving **857,181 bytes** to 1 MiB; two consecutive complete verifications took **0.10 s / 0.10 s real**, leaving **0.90 s / 0.90 s** to the timing trigger, and matched both protected databases. Neither trigger fired. The manifest and every protected byte remain unchanged from v0.30 close. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.31 · 2026-07-31 — The activation constrained Python 3.11.4 and 3.12.13 lanes each collected **317**, passed **317**, failed **0**, and skipped **0**, with the same one warning. No dependency declaration or constraint changed, so neither trigger fired. The result is re-executed by E0's clean rebuild rather than inherited from v0.30. |
| published-release divergence | **Accepted under the operator-selected bound — 2026-07-31** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles, or acquires any public-surface change | v0.31 · 2026-07-31 — At activation v0.31 is open, so the bounded internal `/ingest` diagnostic difference has persisted through **two of three** post-correction closed cycles, v0.29 and v0.30. It has not yet persisted through a third closed cycle, and no public-surface change is present. The trigger has not fired; Step 2 still requires a fresh operator disposition before implementation. |
