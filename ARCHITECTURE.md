...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.33 · 2026-08-01 — E0 measured the exact delivered v0.32 audit child `70b7f93c94c67e43f6f4a29ede5823081955f3fa` at **2,734,366 bytes / 153 files** and confirmed the separately governed closing-visible export of **2,706,393 bytes**, leaving **293,607 bytes / 9.79%** at that governed evaluation point. The same-kind governed→governed series is **2,629,379 → 2,706,393**, or **77,014 bytes/cycle**, so the honest closing-carrying margin is **3.81 cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`293607`; cycles=`3.81`. The historical 5.65 estimate correctly divided a mixed-kind subtraction and is corrected forward as a criterion/evaluation-point error. `export_check` retained exactly v0.30–v0.32 at the measured delivered tree, rejected both excluded byte classes, and reported **100 derived / 7 required** paths; v0.33 activation advanced the configured retained set to v0.31–v0.33. The export trigger did not fire. Governed review-export bytes: `2706393`. |
