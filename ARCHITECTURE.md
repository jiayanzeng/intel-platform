...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export meets or exceeds the executable two-governed-growth-cycle attention boundary, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, any exported path is not Git-tracked, the exact configured observation exclusions differ from the nonempty derived raw-wire class, any derived raw-wire body reappears, or any manifest-derived structural-archive byte reappears | v0.39 · 2026-08-04 — ACTIVATE's exact commit produced an export of **2,730,852 bytes** across **160 files / 2 retained cycles**. EXPORT-TRUTH's exact implementation commit measures **2,675,532 bytes / 158 Git-tracked files / 2 retained cycles**. The exhaustive partition is **158 tracked / 0 untracked defects**; all **158/158** derived required paths are present; the **4** pinned-observation `binary` raw-wire paths equal the **4** exact configured observation exclusions in both directions and are absent; both manifest-derived structural archives are absent. The three untouched untracked amendment inputs are excluded by one bounded historical-family Repomix pattern and remain visible in Git status. Review-export attention boundary: headroom_cycles=`2`; denominator_bytes_per_cycle=`107226`; boundary_bytes=`2785548`. The two-cycle reserve is `3,000,000 - (2 × 107,226)`; the exact implementation export is **110,016 bytes** below it, so the attention predicate did not fire. The derivation is falsified rather than guessed if no positive adjacent governed-growth pair exists or the reserve does not land strictly inside the ceiling. Criterion correction: the former ceiling-only trigger fired after the accepted property had already failed; the new predicate fires while two governed-growth cycles remain. The 3,000,000-byte ceiling itself is unchanged. The governed activation figure remains the latest progress-backed open-cycle value and leaves **269,148 bytes / 8.97% / 2.51 cycles** at the latest +107,226-byte adjacent-cycle denominator. The latest positive adjacent distinct-cycle progress-backed governed denominator remains **2,674,055 → 2,781,281 (+107,226)** across v0.37→v0.38. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.37.md`; prior_bytes=`2674055`; current_progress=`docs/cycles/PROGRESS-v0.38.md`; current_bytes=`2781281`; evaluated_progress=`docs/cycles/PROGRESS-v0.39.md`; evaluated_bytes=`2730852`; denominator_bytes_per_cycle=`107226`; numerator_bytes=`269148`; cycles=`2.51`. Neither attention, retention, tracked-entry, raw-wire, nor structural-archive trigger fired. Governed review-export bytes: `2730852`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.39 · 2026-08-04 — ACTIVATE validates schema v2 at **3 artifacts / 339 pinned files** and measures the manifest at **200,440 bytes**, leaving **848,136 bytes** to its governed boundary. The latest complete checks matched both structural archives, all other pins, and all three protected databases in **0.10 s / 0.11 s real**, leaving **0.90 s / 0.89 s** to the timing trigger. Neither boundary nor timing trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.39 · 2026-08-04 — CEILING-TRIGGER's complete Python 3.11 and Python 3.12 populations each pass **380/380** with the warning non-fatal. The cycle through CEILING-TRIGGER changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. Neither warning trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.39 · 2026-08-04 — Exact authorized publication of v0.17.5 resets the publication epoch to **0** at closing commit `55045ae481ce8d1ef285522b3c0a57c91fe5cb54`; v0.38's measured runtime-behavior difference is included in that published commit. PUBLISH-V17-5 adds no runtime behavior or public surface, and fresh R15 derivation reports **0 public response-domain differences** across **6 routes / 112 field occurrences**. Neither trigger clause has fired. |
