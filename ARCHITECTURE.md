...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.34 · 2026-08-02 — Step 7 forward-binds the live row to the last governed field visible to the closing tree: exact authenticated candidate `1117dc6db6ec0e55e8c8f078ca8059628f9f8262` at an export of **2,527,180 bytes / 151 files / 2 retained cycles**, with both protected byte classes excluded. Step 5's construction measured the two-cycle lever before selection: baseline tree `8fe4225fb7c53c8d146fb5a0725bdc19983d16de` exported **2,629,024 bytes / 153 files / 3 retained cycles** and construction tree `c19876d08502a8aa4eb33e35d25ce2b7d67f32e5` exported **2,520,904 bytes / 151 files / 2 retained cycles**, an exact **108,120-byte** reclaim. The full last-per-progress governed series is **2,576,273 → 2,629,379 (+53,106) → 2,706,393 (+77,014) → 2,592,441 (−113,952) → 2,527,180 (−65,261)**. The selected latest positive basis still predates the v0.33 State archival; the checker enforces identity and arithmetic but emits that a single adjacent pair has no representativeness guarantee and it cannot detect a basis predating a structural change. On the latest positive adjacent same-kind v0.31→v0.32 basis, the governed **472,820-byte / 15.76%** remainder is **6.14 cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; evaluated_progress=`docs/cycles/PROGRESS-v0.34.md`; evaluated_bytes=`2527180`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`472820`; cycles=`6.14`. The 3,000,000-byte ceiling remains an executable repository proxy for a project-knowledge capacity problem last observed externally in v0.28 as **2,067 chunks / 2,000 limit** on a 4,975,987-byte export; the repository has no chunker or current project-knowledge index and cannot recalibrate the proxy against that referent. Neither byte trigger fired. Governed review-export bytes: `2527180`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.34 · 2026-08-02 — Step 6 reverified the unchanged **192,042-byte / 332-pin** manifest at exact authenticated candidate `1117dc6db6ec0e55e8c8f078ca8059628f9f8262`: the local verifier matched every pin, the exact structural State archive, and both protected databases, while hosted `core` independently passed schema validation. No registration or protected-byte write occurred. The latest complete timed pair remains Step 1's **0.12 s / 0.10 s real**, leaving **0.88 s / 0.90 s** to the timing trigger. The latest same-kind positive delta remains **647 bytes/cycle** and the remaining **856,534-byte** margin is **1,323.85 cycles** at that denominator. Neither trigger fired. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.34 · 2026-08-02 — Step 6 exact-candidate local Python 3.11.4 and 3.12.13 lanes each collected/passed **352**, failed **0**, and skipped **0**, with the same one accepted warning. Each hosted lane collected **352**, passed **351**, and skipped the same named, reasoned `on_site` test; for each local/hosted comparison `tools/test_population.py` derived `collected=352`, `equivalent=true`, and `equivalent_passed=352`. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and the warning did not become an error. Neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.34 · 2026-08-02 — Step 7 classified the closing worktree over activation commit `bb4257000cd6a752e807af9f48d0fe871e20d216` exclusive and against published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. The v0.34 path set contains lifecycle documentation, tests, controls, and retained-cycle configuration, but no production source, workflow, dependency, release value, measured runtime-behaviour difference, or public-surface change. Published v0.17.1 reset the count to zero at that closing commit; the count remains zero and no fresh count has started. Final direct remote inspection left `main` and peeled `v0.17.1` at the published closing commit, annotated object `14912f134e45277e2b4fd10b7f5bf8b4900ca20d`, and evidence ref `codex/v0.34-evidence-1117dc6` at exact candidate `1117dc6db6ec0e55e8c8f078ca8059628f9f8262`, all unchanged. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
