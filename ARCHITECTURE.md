...
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.35 · 2026-08-02 — Step 1 reverified the unchanged **192,042-byte / 332-pin** manifest twice; every pin, the exact structural State archive, and both protected databases matched in **0.11 s / 0.10 s real**, leaving **0.89 s / 0.90 s** to the timing trigger. No registration or protected-byte write occurred. The latest same-kind positive delta remains **647 bytes/cycle** and the remaining **856,534-byte** margin is **1,323.85 cycles** at that denominator. Neither trigger fired; wall-clock timing remains outside the lifecycle byte rule. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.35 · 2026-08-02 — Step 1 cleanly rebuilt the constrained Python 3.11.4 and 3.12.13 environments at the exact 21-package resolution. Their permission-complete lanes each collected and passed **352**, failed **0**, skipped **0**, and emitted the same one warning; `tools/test_population.py` independently derived `collected=352`, `equivalent=true`, and `equivalent_passed=352`. The first sandboxed 3.11 run was an environment non-result because loopback binds and `ps` were denied, alongside the then-expected pre-refresh cycle-identity failure. No dependency declaration or constraint changed, the warning remained a warning rather than an error, and neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.35 · 2026-08-02 — Step 1 classified activation commit `5d51d4106e31a4f67215c6e8e66c19488ff29a46` and its audit child over published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. The v0.35 distance so far contains the active runbook/progress records, cycle declaration, and retained-cycle configuration only; it has no production source, workflow, dependency, release value, measured runtime-behaviour difference, or public-surface change. Published v0.17.1 reset the count to zero and no fresh count has started. Direct remote inspection left `main`, peeled `v0.17.1`, annotated object `14912f134e45277e2b4fd10b7f5bf8b4900ca20d`, and evidence ref `codex/v0.34-evidence-1117dc6` unchanged. |
