...
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.31 · 2026-07-31 — The Step 3 constrained Python 3.11.4 and 3.12.13 lanes each collected **322**, passed **322**, failed **0**, and skipped **0**, with the same one warning. No dependency declaration or constraint changed, so neither trigger fired. The machine comparator derived equivalent populations. |
| published-release divergence | **Accepted under the operator-selected bound — 2026-07-31** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles, or acquires any public-surface change | v0.31 · 2026-07-31 — v0.31 remains open, so the bounded internal `/ingest` diagnostic difference has persisted through only **two of three** post-correction closed cycles, v0.29 and v0.30. No public-surface change is present and the trigger has not fired. The operator nevertheless selected patch release v0.17.1 to ship the correction and executable bindings; Step 8 publication remains separately gated. |
