...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.29 · 2026-07-31 — The completed E0 implementation tree's exact project-root export measured **2,430,678 bytes / 152 files**, leaving **569,322 bytes / 18.98%** headroom. `export_check` retained exactly the v0.27, v0.28, and v0.29 TASKS/PROGRESS pairs, rejected neither excluded byte class, and reported **99 derived / 7 required** paths. This supersedes the earlier activation-compatibility measurement of 2,411,393 bytes without erasing it. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest first reaches **1 MiB**, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.29 · 2026-07-31 — E0 measured **316** pins and an actual **182,774-byte** manifest; two clean complete `./run verify-artifacts` executions took **0.11 s / 0.10 s real** and matched both protected databases. Neither trigger fired. The activation record's unchanged-file byte count of 182,780 was a transcription error and is forward-corrected here. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.29 · 2026-07-31 — Clean constrained Python 3.11.4 and 3.12.13 lanes each collected and passed **303** with zero skips and emitted the same one warning. The repository comparator derived `collected=303`, `equivalent=true`, and `equivalent_passed=303`; no dependency declaration or constraint changed, so neither trigger fired. |
