...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.33 · 2026-08-01 — Step 6 measured exact authenticated candidate `2edb7694c2c6c1498b3903382c37aef68329150d` at an export of **2,592,441 bytes / 153 files**, leaving **407,559 bytes / 13.59%** at the governed evaluation point. The selected State archival makes v0.32→v0.33 a truthful **−113,952-byte** governed transition, so it is not a positive growth denominator. The latest positive adjacent same-kind governed→governed basis remains **2,629,379 → 2,706,393**, or **77,014 bytes/cycle**; evaluating the current remainder on that basis gives **5.29 cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; evaluated_progress=`docs/cycles/PROGRESS-v0.33.md`; evaluated_bytes=`2592441`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`407559`; cycles=`5.29`. The executable control now separates the denominator basis from the evaluated point and requires the latest positive adjacent governed pair, so an archival decrease cannot be misreported or an older favorable pair selected. `export_check` retained exactly v0.31–v0.33, rejected both excluded byte classes, and reported **100 derived / 7 required** paths. The export trigger did not fire. Governed review-export bytes: `2592441`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.33 · 2026-08-01 — Operator-selected Fidelity B added one exact structural archive pin. Step 6 reverified the unchanged **192,042-byte / 332-pin** manifest at the exact authenticated candidate: the complete local entry point matched every pin plus both protected databases, and hosted `core` independently passed manifest validation. No Step 6 registration occurred. The latest two complete timed runs remain Step 5's **0.11 s / 0.10 s real**, leaving **0.89 s / 0.90 s** to the timing trigger. The latest same-kind positive delta is **647 bytes/cycle** and the remaining **856,534-byte** margin is **1,323.85 cycles** at that denominator. Neither trigger fired. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.33 · 2026-08-01 — Step 6 local Python 3.11.4 and 3.12.13 lanes each collected/passed **346**, failed **0**, and skipped **0**, with the same one accepted warning. Each hosted lane collected **346**, passed **345**, and skipped the same named, reasoned `on_site` test; for each comparison `tools/test_population.py` derived `collected=346`, `equivalent=true`, and `equivalent_passed=346`. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and no warning became an error. Neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.33 · 2026-08-01 — Step 6 classified the exact authenticated candidate `2edb7694c2c6c1498b3903382c37aef68329150d` against published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. Its file list contains lifecycle documentation, tests, controls, retained-cycle configuration, the State archive, and its structural manifest pin, but no production source, workflow, dependency, release value, measured runtime-behaviour difference, or public-surface change. Published v0.17.1 reset the count to zero at that closing commit; the count remains zero and no fresh count has started. Final direct remote inspection left `main`, peeled `v0.17.1`, and its annotated object unchanged. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
