...
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.34 · 2026-08-02 — Step 1 reverified the unchanged **192,042-byte / 332-pin** manifest twice. Both complete real-workspace runs matched every pin, the exact structural State archive, and both protected databases in **0.12 s / 0.10 s real**, leaving **0.88 s / 0.90 s** to the timing trigger. The latest same-kind positive delta remains **647 bytes/cycle** and the remaining **856,534-byte** margin is **1,323.85 cycles** at that denominator. Neither trigger fired; no manifest or protected artifact byte changed. The lifecycle byte rule explicitly reports wall-clock timing as out of scope. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.34 · 2026-08-02 — Step 1 clean constrained Python 3.11.4 and 3.12.13 lanes each collected/passed **348**, failed **0**, and skipped **0**, with the same one accepted warning; `tools/test_population.py` derived equal **348-test** populations and `equivalent=true`. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and the warning did not become an error. Neither trigger fired. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.34 · 2026-08-02 — Step 1 compared the activation-exclusive and published distances with published closing commit `f02379f03ccdfd1b019413234f2ad014d169fb04`. v0.34 activation and E0 change only cycle declaration, retention configuration, lifecycle/architecture records, and measurements; no production source, workflow, dependency, release value, measured runtime-behaviour difference, or public-surface value changed. Published v0.17.1 reset the count to zero at its closing commit; the count remains zero and no fresh count has started. Direct remote inspection left `main`, peeled `v0.17.1`, and annotated object `14912f134e45277e2b4fd10b7f5bf8b4900ca20d` unchanged. Firing, runtime classification, and reset remain dated operator adjudications; the executable checker enforces record shape, freshness, and cycle identity only. |
