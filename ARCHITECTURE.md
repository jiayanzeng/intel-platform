...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.29 · 2026-07-31 — The completed RE-MEASURE implementation tree's exact project-root export measured **2,500,164 bytes / 152 files**, leaving **499,836 bytes / 16.66%** headroom. `export_check` retained exactly the v0.27, v0.28, and v0.29 TASKS/PROGRESS pairs, rejected neither excluded byte class, and reported **99 derived / 7 required** paths. This latest value supersedes SCHEDULE-DESIGN's 2,481,321-byte live-row measurement; all earlier measurements remain in their dated records. Any later v0.29 export must supersede this row before close. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest first reaches **1 MiB**, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.29 · 2026-07-31 — After the explicitly authorized Step 7 registration, the latest complete measurement is **331** pins and an actual **191,395-byte** manifest, leaving **857,181 bytes** to 1 MiB; two consecutive complete verifications took **0.10 s / 0.10 s real**, leaving **0.90 s / 0.90 s** to the timing trigger, and matched both protected databases. Neither trigger fired. This supersedes E0's 316-pin observation without erasing its dated record. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.29 · 2026-07-31 — The latest permission-complete shell measurement is RE-MEASURE's **306 collected / 306 passed / 0 skipped**, with the same one warning. No dependency declaration or constraint changed, so neither trigger fired. This later measurement preserves BOUNDARY-BIND's equal 306-node result in its dated record. |
