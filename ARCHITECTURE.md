...
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.33 · 2026-08-01 — Step 4 local Python 3.11.4 and 3.12.13 lanes each collected/passed **345**, failed **0**, and skipped **0**, with the same one accepted warning. `tools/test_population.py` derived `collected=345`, `equivalent=true`, and `equivalent_passed=345` for the two local lanes. No dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value changed from published v0.17.1, and no warning became an error. Neither trigger fired. |
