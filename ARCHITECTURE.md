...
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.39 · 2026-08-04 — The v0.17.5 release-parent full gate remains the latest complete Python 3.11 and Python 3.12 measurement at **373/373** each with the warning non-fatal. The entering tree through ACTIVATE changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. Neither warning trigger fired; E0 owns fresh local population counts. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.39 · 2026-08-04 — The entering publication epoch was reset to **0** by exact v0.17.4 and v0.38 is the first later closed cycle carrying a measured runtime-behavior difference, so the count enters v0.39 at **1**. ACTIVATE adds no runtime behavior, and fresh R15 derivation reports **0 public response-domain differences** across **6 routes / 112 field occurrences**. Neither the three-cycle nor immediate trigger has fired before Grant C executes. |
