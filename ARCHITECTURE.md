...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and three-cycle retention depth — 2026-07-30** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus two immediately prior execution cycles, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.34 · 2026-08-02 — Step 1 re-measured exact delivered audit child `e0ab6964f76b0a919c5214607ef141eb5b118deb` at **2,634,692 bytes / 153 files**, retaining exactly v0.31–v0.33 and excluding both protected byte classes. The latest closing-visible governed evaluation point remains authenticated candidate `2edb7694c2c6c1498b3903382c37aef68329150d` at an export of **2,592,441 bytes / 153 files**, leaving **407,559 bytes / 13.59%**. The full last-per-progress governed series is **2,576,273 → 2,629,379 (+53,106) → 2,706,393 (+77,014) → 2,592,441 (−113,952)**. The selected latest positive basis therefore still predates the v0.33 State archival; the current checker enforces identity and arithmetic but has neither an epoch signal nor a representativeness floor. On that basis the governed remainder is **5.29 cycles**. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.31.md`; prior_bytes=`2629379`; current_progress=`docs/cycles/PROGRESS-v0.32.md`; current_bytes=`2706393`; evaluated_progress=`docs/cycles/PROGRESS-v0.33.md`; evaluated_bytes=`2592441`; denominator_bytes_per_cycle=`77014`; numerator_bytes=`407559`; cycles=`5.29`. The 3,000,000-byte ceiling is an executable repository proxy for a project-knowledge capacity problem last observed externally in v0.28 as **2,067 chunks / 2,000 limit** on a 4,975,987-byte export; the repository has no chunker or current project-knowledge index and cannot recalibrate the proxy against that referent. The export trigger did not fire. Governed review-export bytes: `2592441`. |
