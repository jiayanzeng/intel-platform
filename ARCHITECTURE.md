...
| review-export size and retention bound (v0.28) | **Accepted at a 3,000,000-byte ceiling and two-cycle retention depth — operator selected 2026-08-02** | the project-root review export exceeds **3,000,000 bytes**, its cycle-document set differs from the active cycle plus the immediately prior execution cycle, the pinned SEC RSS body reappears, or any `docs/state-archive/**` byte reappears | v0.37 · 2026-08-03 — STATE-ARCHIVE's project-root delivered worktree export passed at **2,558,258 bytes / 154 files / 2 retained cycles**, with exactly the v0.36–v0.37 task/progress pairs retained and both protected byte classes excluded. This delivered structural baseline is **41,742 bytes** below C10's 2,600,000-byte target and leaves **441,742 bytes / 14.72% / 3.08 cycles** at the latest +143,456-byte adjacent-cycle denominator. The exact Step 3 implementation commit `882c698d607b9a36c253c0f3b0a316772063c90b` is the new governed referent at an export of **2,514,762 bytes / 151 files / 2 retained cycles**. It is **85,238 bytes** below C10's target and leaves **485,238 bytes / 16.17% / 3.38 cycles**. The latest positive distinct-cycle progress-backed governed denominator remains **2,742,486 → 2,885,942 (+143,456)** across v0.35→v0.36; the v0.37 structural reclaim is an evaluated baseline, not a positive growth term. Review-export margin: kind=`governed→governed`; prior_progress=`docs/cycles/PROGRESS-v0.35.md`; prior_bytes=`2742486`; current_progress=`docs/cycles/PROGRESS-v0.36.md`; current_bytes=`2885942`; evaluated_progress=`docs/cycles/PROGRESS-v0.37.md`; evaluated_bytes=`2514762`; denominator_bytes_per_cycle=`143456`; numerator_bytes=`485238`; cycles=`3.38`. Neither byte, retention, nor protected-content trigger fired. Governed review-export bytes: `2514762`. |
| protected evidence-manifest growth (v0.22 G4) | **Accepted with bounds — 2026-07-30** | the manifest reaches its governed artifact byte boundary, or two consecutive clean `./run verify-artifacts` runs each take **≥1.00 s real** | v0.37 · 2026-08-03 — STATE-ARCHIVE validated schema v2 at **2 artifacts / 333 pinned files** and measured the manifest at **193,830 bytes**, leaving **854,746 bytes** to its governed boundary. Consecutive complete checks matched both structural archives, all other pins, and both protected databases in **0.09 s / 0.10 s real**, leaving **0.91 s / 0.90 s** to the timing trigger. The new archive is exact-path registered and structural-pinned; neither boundary nor timing trigger fired. |
| shell `StarletteDeprecationWarning` (v0.22 G5) | **Accepted until trigger — 2026-07-30** | the warning becomes an error/failure, or the next authorized constraints refresh changes FastAPI, Starlette, `httpx`, or `httpx2` | v0.37 · 2026-08-03 — Push-triggered PUBLISH run `30824053490`, attempt 1, completed successfully; both Python 3.11 and 3.12 shell jobs and their pytest steps passed. The v0.17.3 closing commit through current HEAD changes no dependency declaration, constraint, FastAPI, Starlette, `httpx`, or `httpx2` value. Neither warning trigger fired; E0 owns fresh local population counts. |
| published-release divergence | **Accepted under the operator-selected bound; publication-epoch reset defined — 2026-08-01** | the unpublished distance contains a measured runtime behaviour difference persisting across three consecutive closed cycles within the current publication epoch, or acquires any public-surface change | v0.37 · 2026-08-03 — DR7 published exact v0.17.3 closing commit `a5afab9e6842a1b6c00a7d17fdeaa3e254edf80f`, resetting the consecutive unpublished-divergence count to **0** there. DOMAIN-MANIFEST now adds response validation and native schema derivation to the shipped shell, but its complete ten-response witness remains byte-identical at **6,869 bytes / SHA-256 `dfec8ff81d68526dd5468ce22660be9d7678c6a8fdd8e52d6ac921c83371cef3`**, its derived v0.17.4 baseline reports no field/domain difference, and golden remains **11/11**. This instruments already-serialized `/v1/*` names and domains rather than adding one; no measured runtime-behaviour or public `/v1/*` surface difference starts or fires the divergence trigger. |
